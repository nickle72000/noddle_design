...
class RIP{
	...
	private function format_content_image($image,$options)
	{
		...
		if(!empty($image->attributes['class']) && preg_match('/(\s|^)non-responsive(\s|$)/',$image->attributes['class']) === 1) return $image;
		...
		$image->output = sprintf('<span data-picture data-alt="%3$s" %1$s>%2$s</span>',
			...
			implode("\n", $sources),
			isset($image->attributes['alt']) ? $image->attributes['alt'] : '');
	...
	private function attributes_to_string($attributes,$exclude = false,$element = 'img'){
		...
		foreach ($attributes as $key => $value) {
			...
			if(!in_array($key, $exclude) && !is_array($value)) array_push($output,sprintf($format,$key,$value));
		}
